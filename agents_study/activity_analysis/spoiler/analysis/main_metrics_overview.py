...
def bootstrap_median_ci(data, n_bootstrap=10000, confidence=0.95, random_seed=42):
...
def plot_change_size(df: pl.DataFrame):
    ...
    # Calculate bootstrap confidence intervals for total changes
    print("  Calculating bootstrap confidence intervals for total changes...")
    ci_total_lower = []
    ci_total_upper = []
    for agent in agents:
        agent_data = df.filter(pl.col("agent") == agent)["total_changes"].to_numpy()
        lower, upper = bootstrap_median_ci(agent_data, n_bootstrap=10000, confidence=0.95)
        ci_total_lower.append(lower)
        ci_total_upper.append(upper)
    
    # Calculate bootstrap confidence intervals for addition ratio
    print("  Calculating bootstrap confidence intervals for addition ratio...")
    ci_ratio_lower = []
    ci_ratio_upper = []
    for agent in agents:
        agent_data = df_with_ratio.filter(pl.col("agent") == agent)["add_ratio"].to_numpy()
        lower, upper = bootstrap_median_ci(agent_data, n_bootstrap=10000, confidence=0.95)
        ci_ratio_lower.append(lower)
        ci_ratio_upper.append(upper)
    ...
    yerr_total_lower = [val - lower for val, lower in zip(median_total, ci_total_lower)]
    yerr_total_upper = [upper - val for val, upper in zip(median_total, ci_total_upper)]
    ...
    # Add value labels on top of bars (accounting for error bars)
    for i, (bar, val, upper_err) in enumerate(zip(bars1, median_total, yerr_total_upper)):
        ax1.text(bar.get_x() + bar.get_width()/2, bar.get_height() + upper_err + max(median_total) * 0.02,
                 f'{val:.0f}', ha='center', va='bottom', fontweight='bold', 
                 fontsize=10)
    
    # ===== BOTTOM PLOT: Addition Ratio =====
    # Calculate error bar sizes
    yerr_ratio_lower = [val - lower for val, lower in zip(median_ratios, ci_ratio_lower)]
    yerr_ratio_upper = [upper - val for val, upper in zip(median_ratios, ci_ratio_upper)]
    ...
    bars2 = ax2.bar(x, median_ratios, width, color=colors_bottom, alpha=0.8,
                    ...
                    yerr=[yerr_ratio_lower, yerr_ratio_upper],
                    capsize=4, error_kw={'linewidth': 1.5})
    ...
    ax2.set_ylim(0, 1.05)  # Ratio is between 0 and 1, add some headroom for error bars
    ax2.grid(True, alpha=0.3, axis='y')
    
    # Add value labels on top of bars (accounting for error bars)
    for i, (bar, val, upper_err) in enumerate(zip(bars2, median_ratios, yerr_ratio_upper)):
        ax2.text(bar.get_x() + bar.get_width()/2, bar.get_height() + upper_err + 0.02,
